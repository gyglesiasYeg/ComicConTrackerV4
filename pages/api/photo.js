// Comic Con Tracker V10 - Stable Known Person Photo API
const KNOWN={
...
async function json(url){const r=await fetch(url,{headers:{'user-agent':'ComicConTracker/11.0','accept':'application/json'}});if(!r.ok)throw new Error('HTTP '+r.status);return r.json()}
