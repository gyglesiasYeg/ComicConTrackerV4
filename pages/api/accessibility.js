// Comic Con Tracker V16.3 - accessibility information lookup
// Reads official convention/venue pages and summarizes accessibility-related text only.
const FIRECRAWL_API_KEY = process.env.FIRECRAWL_API_KEY || '';
const EXTRA_SOURCES = {
  edmonton: ['https://fanexpohq.com/edmontonexpo/', 'https://www.edmontonexpocentre.com/'],
  calgary: ['https://fanexpohq.com/calgaryexpo/', 'https://www.calgarystampede.com/stampede-park'],
  nycc: ['https://www.newyorkcomiccon.com/', 'https://www.newyorkcomiccon.com/en-us/about/accessibility.html', 'https://javitscenter.com/']
};
const TOPICS = [
...
async function scrape(url) {
  ...
  try { const r = await fetch(url,{headers:{'user-agent':'ComicConTracker/16.3 accessibility'}}); if(!r.ok) throw new Error(String(r.status)); return {raw:await r.text(),method:'direct'}; }
...
function links(raw, base) {
  ...
  for (let u of hrefs) {
    ...
    if (/accessib|ada|faq|help|attend|visit|venue|guide|plan/.test(l) && !/facebook|instagram|google|mailto:/.test(l)) out.add(u.split('#')[0]);
  }
  return [...out].slice(0,8);
}
function snippet(text, rx) {
  const t = clean(text); const m = rx.exec(t); if (!m) return '';
  const start = Math.max(0, m.index - 160); const end = Math.min(t.length, m.index + 420);
...
export default async function handler(req,res){
  ...
  const queue=[...new Set(base)]; const checked=[]; let bestSource=''; let bestMethod='none';
  const found = new Map(TOPICS.map(([name])=>[name,'Not found']));
  for (const url of queue.slice(0,10)) {
    const pg=await scrape(url); checked.push(`${pg.method}:${url}`); if(pg.raw && !bestSource){bestSource=url; bestMethod=pg.method;}
    for (const [name,rx] of TOPICS) if(found.get(name)==='Not found'){const sn=snippet(pg.raw,rx); if(sn) found.set(name,sn)}
    if (queue.length < 14) for (const l of links(pg.raw,url)) if(!queue.includes(l)) queue.push(l);
  }
  const items=[...found.entries()].map(([name,value])=>({name,value}));
  const hitCount=items.filter(x=>x.value!=='Not found').length;
